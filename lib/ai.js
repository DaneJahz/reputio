...
Rules:
- Email body: 3-4 sentences, soft call to action, end the email naturally without any signature block, placeholders, or sign-off lines — the user will add their own signature
- Text: under 160 characters, punchy and direct
- Voicemail: sounds like a real person, 20 seconds when read aloud, end with the callback request — no name or company placeholders at the end
- Use the customer name and job type naturally
- Never sound desperate or robotic
- Never use placeholder brackets like [Your Name] or [Company Name] anywhere in any message`,
      }],
    });
...
- Email: 3-4 sentences with a soft call to action, end naturally without any signature block or placeholders
- Text: under 160 characters
- Voicemail: 20 seconds when read aloud, natural speech, no name or company placeholders
- Never use placeholder brackets like [Your Name] or [Company Name] anywhere in any message`,
      }],
    });
    const raw = message.content[0].text.replace(/^```json\s*/i, "").replace(/```\s*$/i, "").trim();
